...
import { quizApi, kelompokApi } from '@/services/api';
...
export function QuizListPage() {
  ...
  const { data: quizList, isLoading, error } = useQuery({
    queryKey: ['quiz'],
    ...
    <div className="min-h-screen bg-surface-alt">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-text">Daftar Quiz</h1>
            <p className="text-text-muted mt-1">Kelola quiz dan sesi</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="card mb-6">
            <h3 className="text-lg font-semibold text-text mb-4">Buat Quiz Baru</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-text mb-1.5">Nama Quiz</label>
                <input
                  type="text"
                  value={nama}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-text truncate">{quiz.nama}</h3>
                    {quiz.deskripsi && (
                      <p className="text-sm text-text-muted mt-1 line-clamp-2">{quiz.deskripsi}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-border-light">
                  <div className="flex items-center gap-1 text-xs text-text-muted">
                    <Users size={14} />
                    <span>{quiz.kelompok?.length ?? 0} kelompok</span>
                  </div>
                  <div className="flex items-center gap-1 text-xs text-text-muted">
                    <Clock size={14} />
                    <span>{quiz.sesi?.length ?? 0} sesi</span>
                  </div>
                </div>
                <div className="mt-3">
                  <span className="text-xs text-secondary font-medium">Lihat detail →</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        {!isLoading && !error && quizList?.length === 0 && (
          <div className="text-center py-12">
            <BookOpen size={48} className="mx-auto text-text-muted mb-3" />
            <p className="text-text-muted">Belum ada quiz. Buat quiz pertama Anda.</p>
          </div>
        )}
      </main>
    </div>
